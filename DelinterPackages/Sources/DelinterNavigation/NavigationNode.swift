...
private extension NavigationNode {
    ...
    var isPushNavigationActive: Binding<Bool> {
        guard case .route(.push, _, _, _, _) = nextNode else {
    ...
    var isSheetPresented: Binding<Bool> {
        guard case .route(.present, _, _, _, _) = nextNode else {
            return .constant(false)
        }
        return isNavigationActive
    }
    ...
    var onDismiss: (() -> Void)? {
        guard case let .route(.present(_, _, onDismiss), _, _, _, _) = nextNode else {
            return nil
        }
        return onDismiss
    }
