// Copyright © 2022 Medly. All rights reserved.
...
public class NavigationStackManager<Screen: Equatable>: ObservableObject {
    ...
    public func present(_ screen: Screen, inNavigation: Bool = true, onDismiss: (() -> Void)? = nil) {
        stack.append(.present(screen, inNavigation: inNavigation, onDismiss: onDismiss))
    }

    /// Pops the last view off the stack (if currently pushed).
    public func pop() {
        guard stack.last?.isPresented == false else { return }
        goBack(by: 1)
    }
    ...
    public func popToRoot(andRefresh refresh: Bool = false) {
        if refresh, let first = stack.first {
            updateRoot(first.screen)
        } else {
            goBack(by: stack.count - 1)
        }
    }

    /// Dismisses the last view from the stack (if currently presented).
    public func dismiss() {
        guard stack.last?.isPresented == true else { return }
        goBack(by: 1)
    }
    ...
    private func goBack(by numberOfScreens: Int) {
        guard stack.count > numberOfScreens else { return }
        stack = stack.dropLast(numberOfScreens)
    }
}
